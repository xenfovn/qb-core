...
function QBCore.Functions.Addalert(title, desc, coords)
    TriggerServerEvent("qb-core:server:addalert", title, desc, coords)
end

CreateThread(function()
    RegisterFontFile('out') -- out is gfx file font  -- PatrickHand
    
    QBCore.FontId = RegisterFontId('Balo')
...
--[[ function QBCore.Functions.GetVehiclesInArea(coords, maxDist)
    local vehiclearea = {}
    local vehicles = GetGamePool('CVehicle')
    for i=1, #vehicles, 1 do
        local vehicleCoords = GetEntityCoords(vehicles[i])
        ...
        if distance < maxDist then
            vehiclearea[#vehiclearea+1] = vehicles[i]
        end
    end
    return vehiclearea
end ]]
