...
local function OnPlayerConnecting(name, setKickReason, deferrals)
    ...
    deferrals.update(string.format('Xin Chào %s. Đang Kiểm Tra Bạn . Đợi Một Chút.', name))
    ...
    deferrals.update(string.format('Chào mừng %s đến với {Server Name}.', name))

    if not license then
        deferrals.done('Không Xác Định Được Rockstar của bạn')
    elseif isBanned then
        deferrals.done(Reason)
    elseif isLicenseAlreadyInUse then
        deferrals.done('Tìm thấy giấy phép rockstar trùng lặp')
    else
        deferrals.done()
        Wait(1000)
...
RegisterNetEvent('QBCore:server:CloseServer', function(reason)
    ...
    if QBCore.Functions.HasPermission(src, 'admin') or QBCore.Functions.HasPermission(src, 'god') then
        ...
        QBCore.Functions.Kick(src, 'bạn không có quyền làm việc này..', nil, nil)
    end
end)
...
RegisterNetEvent('qb-core:server:addalert', function(title, desc, coords)
    local src = source
    local players = QBCore.Functions.GetQBPlayers()
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'police' then
            local alertData = {title = ''..title..'', coords = {coords.x, coords.y, coords.z}, description = desc}
            TriggerClientEvent("qb-phone:client:addPoliceAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('police:client:policeAlert', v.PlayerData.source, coords, desc)
        end
    end
end)
