...
-- membership command
QBCore.Commands.Add('ms', 'Kiểm tra membership', {name = 'id', help = 'id của người cần kiểm tra'}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    local PlayerMeta = Player.PlayerData.metadata['membership']
    if Player then
        if PlayerMeta == false then
            TriggerClientEvent('QBCore:Notify', src, 'Người Chơi Này Không Có Membership', 'error')
        elseif PlayerMeta == true then
            TriggerClientEvent('QBCore:Notify', src, 'Người Chơi Này Có Membership', 'success')
        else
            TriggerClientEvent('QBCore:Notify', src, 'không thể xác định', 'error')
        end
    else
...
QBCore.Commands.Add('setms', 'set membership', {name = 'id', help = 'id của người cần đặt'}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    local PlayerMeta = Player.PlayerData.metadata['membership']
    if Player then
        if PlayerMeta == false then
            Player.Functions.SetMetaData('membership', true)
            TriggerClientEvent('QBCore:Notify', src, 'Đã Cấp Membership', 'success')
        elseif PlayerMeta == true then
            Player.Functions.SetMetaData('membership', false)
            TriggerClientEvent('QBCore:Notify', src, 'Đã Xóa Membership', 'error')
        else
            TriggerClientEvent('QBCore:Notify', src, 'không thể xác định', 'error')
        end
    else
